...
const colorClasses: Record<string, { bg: string; text: string; border: string; dot: string }> = {
  ...
  'primary': { bg: 'bg-teal-500/20', text: 'text-teal-400', border: 'border-teal-500/30', dot: 'bg-teal-500' },
  'chart-1': { bg: 'bg-orange-500/20', text: 'text-orange-400', border: 'border-orange-500/30', dot: 'bg-orange-500' },
  'chart-2': { bg: 'bg-blue-500/20', text: 'text-blue-400', border: 'border-blue-500/30', dot: 'bg-blue-500' },
  'chart-3': { bg: 'bg-purple-500/20', text: 'text-purple-400', border: 'border-purple-500/30', dot: 'bg-purple-500' },
  'chart-4': { bg: 'bg-amber-500/20', text: 'text-amber-400', border: 'border-amber-500/30', dot: 'bg-amber-500' },
  'chart-5': { bg: 'bg-rose-500/20', text: 'text-rose-400', border: 'border-rose-500/30', dot: 'bg-rose-500' },
}
...
export function TimelineCard({ event, category, categories, tags, onDelete, onUpdate, onAddTag }: TimelineCardProps) {
        ...
        <CardContent className="p-4">
          ...
          <div className="flex items-start justify-between gap-2 mb-3">
            ...
            <div className="text-right">
              <p className="text-sm font-medium text-foreground">{formatPeriod(event)}</p>
            </div>
          </div>

          {/* Category Badge */}
          <Badge variant="outline" className={cn('mb-2 text-xs', colors.bg, colors.text, colors.border)}>
            {category.name}
          </Badge>
                ...
                <AlertDialog>
                  ...
                  <AlertDialogContent>
                    <AlertDialogHeader>
                      ...
                      <AlertDialogDescription>
                        Esta acao nao pode ser desfeita. O marco &ldquo;{event.title}&rdquo; sera removido permanentemente da sua linha do tempo.
                      </AlertDialogDescription>
                    </AlertDialogHeader>
                    <AlertDialogFooter>
